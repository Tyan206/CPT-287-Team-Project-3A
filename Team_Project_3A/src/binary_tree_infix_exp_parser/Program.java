...
public class Program {
	...
	public static BTNode postfixToTree(String postfix) {
		BTNode expression = null, root = null;
		Stack<BTNode> stk = new Stack<BTNode>();
		Scanner scanner = new Scanner(postfix);

		while(scanner.hasNext()) {
			root = new BTNode(scanner.next());
			if(Character.isDigit(root.data.charAt(0))) {
				stk.push(root);
				//System.out.println(root.data);
			}else {
				//System.out.println(root.data);
				BTNode rightNode = stk.pop();
				root.right =  rightNode;
				BTNode leftNode = stk.pop();
				root.left =  leftNode;
				stk.push(root);
			}
		}
	...
	public static BTNode infixToTree(String input) {

		// Stack to hold operands stN
		Stack<BTNode> operands = new Stack<>();
		// Stack to hold operators stC
		Stack<String> operators = new Stack<>();

		BTNode root, rightNode, leftNode;
...
/** Evaluates an expression using a expression tree. 
	...
	*/
	...
	public static void main(String[] args) {
		...
		BTNode test = postfixToTree(postfix);
		BTNode test1 = infixToTree(second);
